...
export function PlayerCard({ player, index }: { player: Player; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group bg-card rounded-xl border border-border p-4 hover:border-primary/30 transition-all duration-300 relative overflow-hidden"
    >
      {/* Top glow on hover */}
      <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-primary/0 via-primary/0 to-primary/0 group-hover:via-primary/60 transition-all duration-500" />
      ...
      <div className="grid grid-cols-3 gap-2 text-[10px] text-muted-foreground mb-3">
        ...
        <div className="truncate">{player.iplTeam !== 'None' ? player.iplTeam : 'Uncapped'}</div>
      </div>
      ...
      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button
          ...
          disabled={isInCompare || (!!compareSlots[0] && !!compareSlots[1])}
          className="flex-1 h-7 text-xs"
        >
